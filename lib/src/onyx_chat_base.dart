...
/// Expected type for the optional prefix handler function.
typedef PrefixHandlerFunction = FutureOr<String?> Function(String);

final _onyxLogger = Logger("Onyx");

/// Base class for the Onyx Chat framework.
class OnyxChat {
  ...
  /// Creates OnyxChat with INyxx (websocket or rest) for handling the sending of events.
  ///
  ...
  /// Default prefix handler that is used if a custom one is not passed upon class instantiation.
  ///
  ...
  /// Dispatches a raw message (JSON map) to trigger a Command and/or it's Subcommand.
  ///
  ...
  /// this specific message, bypassing the default prefix or prefix handler.
  Future<void> dispatchRawMessage(Map<String, dynamic> rawJson, {String? messagePrefix}) async {
    ...
    dispatchIMessage(convertedMessage, messagePrefix: messagePrefix);
  }
